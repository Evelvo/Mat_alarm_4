...
def send_notif(tittel, hoved):
    API_KEY = "o.CT4gxIXmOkGk2I4j9uNxjSRgqvCgCdJ2"
    text = hoved
    pb = Pushbullet(API_KEY)
    push = pb.push_note(tittel, text)



    
@app.route("/force_mat_alarm")
def force_mat_alarm():
    print(f"{request.remote_addr} opened force website")
    global command_txt
    global set_change_timer
    set_change_timer = True
    send_notif(tittel = "Mat", hoved = "Det er mat")
...
def legg_knapp():
    ...
    global set_change_timer
    set_change_timer = True
    send_notif(tittel = "Legg deg", hoved = "Legg deg om 10 minutter")
...
def møte_knapp():
    ...
    global set_change_timer
    set_change_timer = True
    send_notif(tittel = "Møte", hoved = "Møte")
...
def skjerm_knapp():
    ...
    global set_change_timer
    set_change_timer = True
    send_notif(tittel = "Skjermpause", hoved = "Skjermpause")
...
def data():
    ...
    if request.method == 'POST':
        ...
        print(text)
        send_notif(tittel = "Tilpasset", hoved = text)
        command_txt = text
        set_change_timer = True
